...
def evaluate_baseline(num_episode=1, max_time_step_per_episode=1000, verbose=False):
    ...
    player_policy = make_random_policy(env.np_random)
    total_rewards = []
    for i_episode in range(num_episode):
        total_reward = 0.0
        observation = env.reset()
        assert env.state.color == env.player_color
        # player_policy = make_pachi_policy(board=env.state.board, engine_type=six.b('uct'), pachi_timestr=six.b('_2400'))
        for t in range(max_time_step_per_episode):
            if verbose:
                env.render()
                print(observation)
            action = player_policy(env.state, None, None)
...
num_episods = 10
t0 = time.time()
total_rewards = evaluate_baseline(num_episode=num_episods, verbose=False)
