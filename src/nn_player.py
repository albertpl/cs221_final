...
class NetworkPlayer(object):
    def __init__(self, config: ModelConfig, player_color):
        assert player_color in ('black', 'white')
    ...
    def next_action(self, state: GoState, prev_state: GoState, prev_action):
        ...
        player_stone = pachi_py.BLACK if self.player_color == 'black' else pachi_py.WHITE
        _board_channels = [0, 1] if player_stone == pachi_py.BLACK else [1, 0]
        ...
        batch_input.batch_xs[0, ..., -1] = player_stone
