...
from environment import GoEnv, GoState, action_to_coord, resign_action
...
class Player(object):
    def __init__(self, config: ModelConfig):
        assert config.player in (pachi_py.BLACK, pachi_py.WHITE)
        self.config = config
    ...
    def play(self, state: GoState):
        board_size = self.config.board_size
        depth = self.config.tree_depth
        observation = state.board.encode()
        _board_channels = [0, 1] if self.config.player == pachi_py.BLACK else [1, 0]
        ...
        batch_input.batch_xs[0, ..., -1] = self.config.player
        ...
        probabilities = batch_output.result[0]
